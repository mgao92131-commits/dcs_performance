"""Configuration models for the esterification-level rate rule."""
...
def parse_config(raw_config: Mapping[str, Any]) -> LevelRateConfig:
    ...
    for index, raw_point in enumerate(raw_points):
        ...
        history_tag = _text(raw_point.get("history_tag"), f"{prefix}.history_tag")
        smoothing = _parse_smoothing(raw_point.get("smoothing", {}), prefix)
        rate_window = _positive(
            raw_point.get("rate_window_seconds"),
            f"{prefix}.rate_window_seconds",
        )
        lower_rate = _finite(raw_point.get("lower_rate"), f"{prefix}.lower_rate")
        upper_rate = _finite(raw_point.get("upper_rate"), f"{prefix}.upper_rate")
        if lower_rate >= upper_rate:
            raise ValueError(f"{prefix}.lower_rate must be less than upper_rate")
        persistence = _positive(
            raw_point.get("persistence_seconds"),
            f"{prefix}.persistence_seconds",
        )
        max_gap = _positive(
            raw_point.get("max_gap_seconds", 60),
            f"{prefix}.max_gap_seconds",
        )
        merge_gap = _nonnegative(
...
def load_config(source: str | Path | Mapping[str, Any]) -> LevelRateConfig:
